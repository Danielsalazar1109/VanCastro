...
const StepNavigation: React.FC<StepNavigationProps> = ({ currentStep, onStepChange, steps }) => {
	return (
		<div className="flex justify-between items-center w-full mb-6">
			{steps.map((step, index) => (
              ...
              ${index + 1 < currentStep ? "bg-green-400 border-green-500" : ""}
            `}
					>
						<span
							className={`
	...
	return (
      ...
      `}
		>
			...
			<p className="text-sm text-gray-600">{instructor.locations.join(", ")}</p>
...
const EnhancedDatePicker = ({
	...
	return (
		<div className="mb-4">
			<label className="block text-gray-700 text-sm font-bold mb-4">Select Your Lesson Date</label>
...
export default function NewBookingForm({ userId }: NewBookingFormProps) {
	...
	const classTypeOptions = [
		{
			value: "class 4",
			imageUrl:
				"https://media.istockphoto.com/id/865989884/vector/caucasian-bus-driver-sitting-at-steering-wheel.jpg?s=612x612&w=0&k=20&c=zmxXE-jde_xBtGSxqrdOxZWVS7idP6y3FcLiOHWO-cU=",
			alt: "Class 4",
		},
		{
			value: "class 5",
			imageUrl:
				"https://media.istockphoto.com/id/838089460/vector/female-hand-with-driver-license.jpg?s=612x612&w=0&k=20&c=bvednGjVoojSkZKYIjQ2IFR-IcwoqaoAOzzYY-OHmrY=",
			alt: "Class 5",
		},
		{
			value: "class 7",
	...
	const checkPackageCompletion = async () => {
		...
		try {
			...
			const response = await fetch(`/api/booking?userId=${userId}&status=approved`);
			...
			const filteredBookings = bookings.filter(
				(booking: any) =>
					booking.classType === classType && booking.duration === duration && booking.status !== "cancelled"
			);
			...
			if (classType === "class 5" && duration === 90) {
				...
				if (filteredBookings.length === 2) {
					...
					isComplete = true;
					console.log(
						`This is booking #${filteredBookings.length + 1} for Class 5 (90 min), completing another package`
					);
				}
				...
				if (filteredBookings.length === 9) {
					...
					isComplete = true;
					console.log(
						`This is booking #${filteredBookings.length + 1} for Class 7 (60 min), completing another package`
					);
				}
			}
			...
			console.log(
				`Package completion check: isComplete=${isComplete}, packageSize=${pkgSize}, existingBookings=${filteredBookings.length}`
			);
	...
	const calculateDiscountedPrice = async (pkgSize: number) => {
		...
		console.log(
			`Calculating discounted price for packageSize=${pkgSize}, classType=${classType}, duration=${duration}`
		);
		...
		try {
			...
			const packagePriceRecord = prices.find(
				(p) => p.classType === classType && p.duration === duration && p.package === packageType
			);
			...
			if (packagePrice) {
				...
				console.log(
					`Calculated discounted price: $${individualPrice} = $${packagePrice} - $${price} * (${pkgSize} - 1)`
				);
	...
	const fetchInstructors = async () => {
		try {
			setLoading(true);
			const response = await fetch(`/api/instructors?location=${location}&classType=${classType}`);
	...
	const fetchSchedules = async () => {
		try {
			...
			if (data.schedules && data.schedules.length > 0) {
				...
				const availableSlots = schedule.slots.filter((slot: TimeSlot) => !slot.isBooked);
			...
			if (
				error.message.includes("Instructor is not available") ||
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const selectedSlot = availableTimeSlots.find((slot) => `${slot.startTime}-${slot.endTime}` === timeSlot);
			...
			console.log(
				`Submitting booking with price: $${finalPrice} (isPackageComplete: ${isPackageComplete}, discountedPrice: ${discountedPrice}, regular price: ${price})`
			);
		...
		return (
			<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
				<div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
					...
					<p className="mb-6">
						Class 7 lessons require that you have passed the knowledge test. Have you already passed your
						knowledge test?
					</p>
		...
		<div className="w-full max-w-2xl mx-auto bg-gradient-to-b from-white to-yellow-50 p-10 rounded-xl shadow-lg border border-yellow-200">
			...
			{hasPassedKnowledgeTest === false && (
				<div className="mb-4 p-3 bg-yellow-100 text-yellow-800 rounded-md">
					You need to pass the knowledge test before booking a Class 7 lesson. Please select a different class
					type or complete your knowledge test first.
				</div>
			)}
			...
			<form onSubmit={handleSubmit}>
				{step === 1 && (
					<div className="space-y-8">
						...
						<div className="mb-6">
							<div className="bg-yellow-50 border-2 border-yellow-300 p-4 rounded-lg">
								...
								<ul className="list-disc pl-5 space-y-2 text-sm text-gray-700">
									<li>
										<span className="font-semibold">Class 7 (60 minutes):</span> After accumulating
										10 lessons, you'll receive a discount!
										<span className="text-green-600 font-medium"> $892.50</span> (Regular price:
										$945)
									</li>
									<li>
										<span className="font-semibold">Class 5 (90 minutes):</span> After accumulating
										3 lessons, you'll receive a discount!
										<span className="text-green-600 font-medium"> $262.50</span> (Regular price:
										$283.50)
									</li>
									<li>
										<span className="font-semibold">Road Test (2 hours):</span> Available as a
										single lesson
									</li>
								</ul>
								<p className="text-sm text-gray-700 mt-3 italic">
									The system will automatically track your lessons and apply discounts when you reach
									the required number.
								</p>
							</div>
						</div>
						...
						<div className="mb-8">
							<label className="block text-gray-700 text-sm font-bold mb-4">Duration</label>
							<div className="grid grid-cols-3 gap-3 sm:gap-5">
								{durations.map((dur) => (
									<label key={dur} className="flex flex-col items-center">
										<input
											type="radio"
											name="duration"
											value={dur}
                        ...
                        ${
							duration === dur
								? "bg-yellow-400 border-yellow-500 scale-105 shadow-md"
                      ...
                      `}
										...
										<div className="mt-2 text-center">
											<span className="text-sm sm:text-base font-medium text-gray-800">
												{dur === 120 ? "Road Test" : `${dur} minutes`}
											</span>
											{dur === 60 && (
												<p className="text-xs text-gray-600 mt-1">Perfect for beginners</p>
											)}
											{dur === 90 && (
												<p className="text-xs text-gray-600 mt-1">Most popular choice</p>
											)}
											{dur === 120 && (
						...
						<div className="mb-6">
							<div>
								...
								<p className="text-sm text-gray-600 mb-4">
									These time slots are automatically generated based on the instructor's availability
									for{" "}
									{date
								...
								{availableTimeSlots.length > 0 && !timeSlot && (
									<p className="text-sm text-red-500 mt-3">Please select a time slot to continue.</p>
								)}
							</div>
						</div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
								...
								<div className="p-2">
									...
									<p className="mb-2">
										...
										<span className="text-gray-900">{timeSlot}</span>
									</p>
									{classType === "class 7" && (
										<p className="mb-2">
											<span className="font-semibold text-gray-700">Knowledge Test:</span>{" "}
