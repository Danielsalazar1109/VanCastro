...
export default function InstructorDashboard() {
  ...
  useEffect(() => {
    if (instructorId) {
      if (activeTab === "bookings") {
        // Only set selected date to today and fetch bookings for today
        // when the component first loads or when switching to the bookings tab
        ...
        if (isFirstLoad) {
          ...
          // Set selected date to today only on first load
          setSelectedDate(todayFormatted);
          
          // Fetch bookings for today
          fetchBookingsForDate(todayFormatted);
          
          // Mark initial data as loaded
          setInitialDataLoaded(true);
        } else {
          // On tab switch, fetch bookings for the currently selected date
  ...
  }, [instructorId, activeTab]); // Removed calendarEvents from dependencies

  // Optimized function to fetch bookings for a specific date
  const fetchBookingsForDate = async (date: string) => {
    ...
    try {
      ...
      const cacheKey = `${instructorId}_${date}`;
      const currentTime = Date.now();
      ...
      // Fetch approved bookings for this instructor for the specified date
  ...
  useEffect(() => {
	...
	const generateDates = () => {
		...
		const startDay = isMobile ? -1 : -3; // For mobile: yesterday, today, tomorrow
		const endDay = isMobile ? 1 : 3;
	...
	// Removed duplicate useEffect hook to fix syntax errors and optimize loading
	...
	const fetchInstructorId = async (email: string) => {
		try {
			...
			const instructor = instructorData.instructors.find((i: any) => i.user._id === userId);
	...
	const fetchBookings = async (showLoading = true) => {
		try {
			...
			// Check cache first
			const cacheKey = `all_bookings_${instructorId}`;
			const currentTime = Date.now();
			const cachedData = bookingsCache[cacheKey];
			const isCacheValid = cachedData && (currentTime - cachedData.timestamp < 5 * 60 * 1000);
			...
			// Fetch data directly without timeout
					...
					fullAvailability.forEach((slot) => {
						if (slot.isAvailable) {
							const startTimeWithSeconds =
								slot.startTime.includes(":") && slot.startTime.split(":").length === 2
									? `${slot.startTime}:00`
									: slot.startTime;

							const endTimeWithSeconds =
								slot.endTime.includes(":") && slot.endTime.split(":").length === 2
									? `${slot.endTime}:00`
									: slot.endTime;
	...
	// Optimized function to filter bookings by selected date - uses server-side filtering
		...
		<>
			<div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4 md:p-10">
				...
				<div className="md:hidden flex justify-between items-center mb-4">
					...
					<button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-yellow-600">
				...
				<div className="hidden md:flex text-4xl font-bold mb-6">Welcome {session?.user?.name} 👋</div>
												...
												>
													...
													<div className="mb-3">
														...
														<p className="text-current/70">
															{booking.classType} - {booking.duration} mins
														</p>
													</div>
			...
			{/* Booking Modal */}
			<BookingModal booking={selectedBooking} isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
